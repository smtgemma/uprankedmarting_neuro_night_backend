...
const getAllAgents = async (query: Record<string, unknown>, options: IPaginationOptions) => {
  ...
  if (query.isAssigned !== undefined) {
    if (query.isAssigned === "true" || query?.isAssigned === true) {
      // Agents with at least one assignment
      whereConditions.assignments = {
        some: {
          status: AssignmentStatus.ASSIGNED,
        },
      };
    } else if (query.isAssigned === "false" || query?.isAssigned === false) {
  ...
  // Filter by organization (agents assigned to specific org)
...
const getAgentsByOrganization = async (
  organizationId: string,
...
const removeAgentFromOrganization = async (payload: {
  ...
  if (assignment.status === AssignmentStatus.REMOVED) {
    throw new AppError(
      status.BAD_REQUEST,
      "Agent is already removed from this organization!"
    );
  }

  // Update assignment status to REMOVED
  const result = await prisma.agentAssignment.update({
    where: {
      agentUserId_organizationId: {
        agentUserId: agentUserId,
        organizationId: organizationId,
